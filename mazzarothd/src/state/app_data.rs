...
pub fn get_block_db_path() -> anyhow::Result<String> {
    ...
    Ok(path.to_str().ok_or_else(|| anyhow::anyhow!("Failed to get block db path"))?.to_string())
}
...
pub fn get_mvm_db_path() -> anyhow::Result<String> {
    ...
    Ok(path.to_str().ok_or_else(|| anyhow::anyhow!("Failed to get mvm db path"))?.to_string())
}